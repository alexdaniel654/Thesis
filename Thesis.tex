...
\thispagestyle{plain}
%\lipsum[1]
%Renal MRI's primary use in the clinic is to provide structural information about the kidneys. Presented below is a quantitative method for measuring the rate at which the kidneys are using oxygen. This method, known as $T_2$ Relaxation Under Spin Tagging (TRUST) has previously been used in the brain and here is successfully used to measure oxygen saturation in the renal vein. A key step in enabling the use of quantitative MRI in the clinic is understanding the interplay between the current histological analysis and the novel MR measures. Outlined here are the first stages of a project to better understand these correlations by scanning samples in-vivo, ex-vivo and carrying out volume matched histology.
