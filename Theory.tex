...
To measure \ttwo, rather than \ttwostar, the effects of static $B_0$ inhomogeneities that lead to $T_2'$ must be negated. Because the de-phasing effects leading to $T_2'$ are constant over time, the refocussing effects of a \ac{SE} sequence can be utilised to reform this de-phasing component. In a \ac{SE} sequence, an initial 90\degree{ } pulse shifts $M$ into the transverse plane and induces phase coherence, Figure \ref{fig:theory_se_spins_t0}. $T_2'$ effects will then cause some spins to precess quickly and others more slowly and thus de-phase with $T_2'$, Figure \ref{fig:theory_se_spins_t_te}. At time, \ac{TE}, later a 180\degree{ } pulse is used to flip the spin ensemble, reversing the phase shift meaning those spins that had accrued the largest positive phase shift will now have the largest negative phase shift and vice versa, Figure \ref{fig:theory_se_spins_t_te_inverted}. Because the $B_0$ inhomogeneities that lead to $T_2'$ are static, they will still be acting to the same degree on each spin. This leads to an echo forming at $t =$ 2\ac{TE} as those spins with the highest Larmor frequency, and largest negative phase shift, refocus or ``catch up'' with those spins with a lower Larmor frequency, Figure \ref{fig:theory_se_spins_t_2te}.
\begin{figure}[H]
	\centering
	...
	\begin{subfigure}[c]{0.24\textwidth}
		...
		\label{fig:theory_se_spins_t_te_inverted}
	\end{subfigure}
	\begin{subfigure}[c]{0.24\textwidth}
		\centering
		\includegraphics[width=1\textwidth]{Theory/SE/se_spins_t_2te.eps}
		\caption{$t$ = 2TE}
		\label{fig:theory_se_spins_t_2te}
	\end{subfigure}
	\caption{Spins evolving in a spin echo sequence showing the de-phasing, (\subref{fig:theory_se_spins_t_te}), refocusing pulse, (\subref{fig:theory_se_spins_t_te_inverted}), and subsequent refocusing, (\subref{fig:theory_se_spins_t_2te}).}
