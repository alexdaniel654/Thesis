...
The initial step in localisation is to measure the signal from a single, spatially defined, slice. This is achieved using gradient fields. These produce small linear perturbations in $B_0$ and are applied in a combination of the $x$, $y$ and $z$ direction to enable arbitrary gradient directions. The resulting $B_0$ field leads to the resonant frequency varying as a function of position. \\

If a gradient is applied along the $z$ direction, $G_z$, the magnetic field experiences by the spins at position $z$ will be
\begin{equation}
B = B_0 + G_zz.
\end{equation}
As such, the Larmor frequency becomes
\begin{equation}
\omega = \gamma\left( B_0 + G_zz\right).
\end{equation}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{(\subref{fig:theory_slice_select_rephasing}) A truncated sinc pulse of bandwidth $\Delta \omega$ being applied over a slice selective gradient followed by the negative re-phasing gradient lobe. Note that here the area under the re-phasing gradient is half of that of the slice selective gradient. (\subref{fig:theory_slice_select_profile}) Example slices of thickness $\Delta z$ being excited by \ac{RF} pulses of bandwidth $\Delta \omega$ showing that excitation pulses of larger bandwidth result in thicker slice profiles.}
