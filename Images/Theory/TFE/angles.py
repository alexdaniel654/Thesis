...
fig, ax = plt.subplots()
ax = sns.heatmap(df, cbar=False, ax=ax, cmap='inferno')
...
cbar = fig.colorbar(plt.imshow(sig, cmap='inferno'))
cbar.ax.set_ylabel('$S_{TFE}$/$M_0$')
ax.plot(np.arange(0.5, 500.5), (ernst/180)*500, label='Ernst Angle', color='C2')
