...
\todo{Add stuff about alternative network architectures (\ac{RNN} and \ac{LSTM}, 3D \ac{CNN} or `colour' \ac{CNN})\cite{chen_combining_2016, stollenga_parallel_2015}}

Future work will collect more training data to compare the 2D \ac{CNN} with a 3D \ac{CNN} to ascertain if the potential increase in accuracy is worth the increased hardware requirements and reduced generalisability. Here, the CNN has been developed for use on a \ttwo-weighted sequence and has not been validated on \tone-weighted images. This image contrast was chosen as a result of recent publications comparing T1- and \ttwo-weighted images for \ac{TKV} assessment reporting that \ttwo-weighted images provide better quality to enable \ac{TKV} measurements, leading to improved reproducibility with lower intra- and inter-reader variability \cite{van_gastel_t1_2018}. \tone-weighted data could be registered to the \ttwo-weighted data and used as an extra channel to inform the segmentation. 
